...
func TestEffectiveTransportTuningSummaryUsesConvergedGenericProfile(t *testing.T) {
	cfg := config.DefaultNetworkConfig()
	cfg.TransportTuning.GenericDownloadTotalBitrateBps = 64 * 1024 * 1024
	cfg.TransportTuning.GenericDownloadRTPBitrateBps = 64 * 1024 * 1024
	cfg.TransportTuning.GenericDownloadRTPSocketBufferBytes = 64 << 20
	cfg.TransportTuning.GenericDownloadRTPReorderWindowPackets = 2048
	cfg.TransportTuning.GenericDownloadRTPLossTolerancePackets = 1024
	cfg.TransportTuning.GenericDownloadRTPGapTimeoutMS = 5000
	cfg.TransportTuning.GenericDownloadRTPFECEnabled = false
	cfg.TransportTuning.GenericDownloadRTPFECGroupPackets = 1
	summary := effectiveTransportTuningSummary(cfg)
	if summary.GenericDownloadRTPBitrateBps != 16*1024*1024 {
		t.Fatalf("bitrate=%d", summary.GenericDownloadRTPBitrateBps)
	}
	if summary.GenericDownloadRTPReorderWindow != 768 || summary.GenericDownloadRTPLossTolerance != 256 || summary.GenericDownloadRTPGapTimeoutMS != 1200 {
		t.Fatalf("generic tolerance summary=%+v", summary)
	}
}
...
func TestDefaultConfigYAMLContainsRequiredSections(t *testing.T) {
	...
	text := string(raw)
	for _, section := range []string{"sip:", "rtp:", "storage:", "observability:", "ui:", "ops:"} {
