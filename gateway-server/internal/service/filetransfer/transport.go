...
type SecurityEventRecorder func(category, transport, reason, requestID, traceID string)

var securityEventRecorder SecurityEventRecorder

func SetSecurityEventRecorder(recorder SecurityEventRecorder) { securityEventRecorder = recorder }

func recordSecurityEvent(category, transport, reason, requestID, traceID string) {
	if securityEventRecorder != nil {
		securityEventRecorder(category, transport, reason, requestID, traceID)
	}
}
...
func (s *TCPSession) WritePacket(packet rtpfile.ChunkPacket) error {
	...
	hdr, err := marshalChunkPacketHeader(packet)
	if err != nil {
		s.transport.metrics.writeErrorsTotal.Add(1)
		return err
	}
	payloadLen := len(hdr) + len(packet.Payload)
	if payloadLen > s.cfg.MaxPacketBytes {
		s.transport.metrics.writeErrorsTotal.Add(1)
		return fmt.Errorf("rtp tcp frame too large: %d > %d", payloadLen, s.cfg.MaxPacketBytes)
	}
	var frameHeader [tcpFrameHeaderSize]byte
	binary.BigEndian.PutUint32(frameHeader[:], uint32(payloadLen))
	buffers := net.Buffers{frameHeader[:], hdr, packet.Payload}
...
func (s *TCPSession) ReadPacket() (rtpfile.ChunkPacket, error) {
	...
	frameBuf := s.readFrameBuf
	if cap(frameBuf) < int(length) {
		frameBuf = make([]byte, int(length))
	} else {
		frameBuf = frameBuf[:int(length)]
	}
	s.readFrameBuf = frameBuf
	if _, err := io.ReadFull(s.conn, frameBuf); err != nil {
		s.transport.metrics.readErrorsTotal.Add(1)
		return rtpfile.ChunkPacket{}, err
	}
	packet, err := unmarshalChunkPacket(frameBuf)
	if err != nil {
		s.transport.metrics.readErrorsTotal.Add(1)
		return rtpfile.ChunkPacket{}, err
	}
...
func marshalChunkPacketHeader(packet rtpfile.ChunkPacket) ([]byte, error) {
	...
	return packet.Header.MarshalBinary()
}
...
func unmarshalChunkPacket(data []byte) (rtpfile.ChunkPacket, error) {
	...
	packet := rtpfile.ChunkPacket{Header: h, Payload: payload}
	if err := rtpfile.ValidatePacket(packet, time.Now().UTC()); err != nil {
		recordSecurityEvent("rtp_packet_rejected", "RTP", err.Error(), string(h.RequestID[:]), string(h.TraceID[:]))
		return rtpfile.ChunkPacket{}, err
	}
	return packet, nil
}
