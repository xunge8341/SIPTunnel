...
func TestRunnerRun_PortConflictOwnedByCurrentProcessIsInfo(t *testing.T) {
	runner := &Runner{
		nowFn:          func() time.Time { return time.Now().UTC() },
		interfaceIPs:   func() (map[string]struct{}, error) { return map[string]struct{}{"127.0.0.1": {}}, nil },
		listenTCP:      func(_, _ string) (net.Listener, error) { return nil, errors.New("address already in use") },
		...
		execCommand: func(_ context.Context, name string, args ...string) ([]byte, error) {
			if name == "lsof" {
				return []byte(fmt.Sprintf(`COMMAND PID USER FD TYPE DEVICE SIZE/OFF NODE NAME
gateway %d root 6u IPv4 0t0 TCP *:5060 (LISTEN)
`, os.Getpid())), nil
			}
			return nil, errors.New("unsupported")
		},
	...
	for _, item := range report.Items {
		...
		if item.Level != LevelInfo {
			t.Fatalf("level=%s, want info", item.Level)
		}
		if !strings.Contains(item.Message, "当前 gateway 进程") {
			t.Fatalf("message missing self-owned hint: %s", item.Message)
		}
		return
	}
	t.Fatalf("sip.listen_port_occupancy not found: %+v", report.Items)
...
func TestRunnerRun_PortConflictExpectedRuntimeOwnershipIsInfo(t *testing.T) {
	runner := &Runner{
		nowFn:          func() time.Time { return time.Now().UTC() },
		interfaceIPs:   func() (map[string]struct{}, error) { return map[string]struct{}{"127.0.0.1": {}}, nil },
		listenTCP:      func(_, _ string) (net.Listener, error) { return nil, errors.New("address already in use") },
	...
	report := runner.Run(t.Context(), Input{NetworkConfig: cfg, StoragePaths: config.StoragePaths{TempDir: "./tmp", FinalDir: "./final", AuditDir: "./audit"}, RunMode: "dev", ExpectSIPPortOwnedByCurrentProcess: true})
	for _, item := range report.Items {
		if item.Name != "sip.listen_port_occupancy" {
			continue
		}
		...
		if !strings.Contains(item.Message, "运行态自检视为正常") {
			t.Fatalf("message missing runtime ownership hint: %s", item.Message)
		}
		return
	}
	t.Fatalf("sip.listen_port_occupancy not found: %+v", report.Items)
