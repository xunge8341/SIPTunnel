...
func Run(ctx context.Context, opts Options) SuiteResult {
	...
	results := []CheckResult{
		...
		runNamedCheck("就绪探针", func() (bool, string) {
			return checkReadyz(ctx, opts.Client, baseURL)
		}),
...
func checkReadyz(ctx context.Context, client *http.Client, baseURL string) (bool, string) {
	body, code, err := getJSON(ctx, client, baseURL+"/readyz")
	if err != nil {
		return false, err.Error()
	}
	if code >= 400 {
		reasons := jsonPathString(body, "message")
		return false, fmt.Sprintf("status=%d %s", code, strings.TrimSpace(reasons))
	}
	status := jsonPathString(body, "data", "status")
	if strings.ToLower(strings.TrimSpace(status)) != "ready" {
		return false, fmt.Sprintf("status=%s", status)
	}
	return true, "readyz=ready"
}
