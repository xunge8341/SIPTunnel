...
const (
	...
	MessageTypeHTTPForwardRequest                = "http.forward.request"
	MessageTypeHTTPForwardResponse               = "http.forward.response"
...
func (h Header) ValidateEnvelope(now time.Time) error {
	if err := validateBoundedField("protocol_version", h.ProtocolVersion, 16); err != nil {
		return err
	}
	if err := validateBoundedField("message_type", h.MessageType, 64); err != nil {
		return err
	}
	if err := validateBoundedField("request_id", h.RequestID, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("trace_id", h.TraceID, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("session_id", h.SessionID, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("api_code", h.ApiCode, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("source_system", h.SourceSystem, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("source_node", h.SourceNode, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("nonce", h.Nonce, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("payload_digest", h.PayloadDigest, maxFieldLength); err != nil {
		return err
	}
	if err := validateBoundedField("signature", h.Signature, maxSignatureBytes); err != nil {
		return err
	}
	if err := validateAllowedValue("digest_alg", h.DigestAlg, map[string]struct{}{
		"SHA256": {},
		"MD5":    {},
	}); err != nil {
		return err
	}
	if err := validateAllowedValue("sign_alg", h.SignAlg, map[string]struct{}{
		"HMAC-SHA256": {},
		"RSA-SHA256":  {},
	}); err != nil {
		return err
	...
	if now.Sub(h.Timestamp) > allowedClockSkew {
		return fmt.Errorf("timestamp is older than allowed skew")
	}
	return nil
}

func (h Header) ValidateForMessageType(expected string, now time.Time) error {
	if err := h.ValidateEnvelope(now); err != nil {
		return err
	}
...
func validateBoundedField(name, value string, maxLen int) error {
	...
	if strings.ContainsAny(v, "\r\n") {
		return fmt.Errorf("%s contains illegal control characters", name)
	}
	return nil
}

func validateAllowedValue(name, value string, allow map[string]struct{}) error {
	v := strings.ToUpper(strings.TrimSpace(value))
	if _, ok := allow[v]; !ok {
		return fmt.Errorf("%s is unsupported: %s", name, value)
	}
	return nil
}
