...
func TestValidateEnvelopeRejectsSkewedTimestamp(t *testing.T) {
	now := uint64(1720000000000)
	h := Header{
		Magic:           Magic,
		...
		ChunkLength:     1,
		FileSize:        1,
		ChunkDigest:     sha256.Sum256([]byte{1}),
		FileDigest:      sha256.Sum256([]byte{1}),
		SendTimestamp:   now + maxClockSkewMs + 1,
	}
	if err := h.ValidateEnvelope(time.UnixMilli(int64(now)).UTC()); err == nil {
		t.Fatal("expected skew validation failure")
	}
}

func TestValidatePacketRejectsMissingIDs(t *testing.T) {
	p := ChunkPacket{
		Header: Header{
			Magic:           Magic,
			ProtocolVersion: ProtocolVersion,
			...
			ChunkDigest:     sha256.Sum256([]byte{1}),
			FileDigest:      sha256.Sum256([]byte{1}),
			SendTimestamp:   uint64(time.Now().UnixMilli()),
		},
		Payload: []byte{1},
	}
	if err := ValidatePacket(p, time.Now().UTC()); err == nil {
		t.Fatal("expected missing id validation failure")
	}
}
