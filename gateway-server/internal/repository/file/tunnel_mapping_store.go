...
func (s *TunnelMappingStore) Create(m tunnelmapping.TunnelMapping) (tunnelmapping.TunnelMapping, error) {
	...
	if err := s.ensureNoConflictsLocked(m, ""); err != nil {
		return tunnelmapping.TunnelMapping{}, err
	}
...
func (s *TunnelMappingStore) Update(id string, m tunnelmapping.TunnelMapping) (tunnelmapping.TunnelMapping, error) {
	...
	if err := s.ensureNoConflictsLocked(m, id); err != nil {
		return tunnelmapping.TunnelMapping{}, err
	}
	m.UpdatedAt = time.Now().UTC().Format(time.RFC3339)
	s.data[id] = m
...
func (s *TunnelMappingStore) ensureNoConflictsLocked(candidate tunnelmapping.TunnelMapping, ignoreID string) error {
	deviceID := strings.TrimSpace(candidate.EffectiveDeviceID())
	for id, item := range s.data {
		if id == ignoreID {
			continue
		}
		if strings.EqualFold(strings.TrimSpace(item.EffectiveDeviceID()), deviceID) {
			return fmt.Errorf("resource_code/device_id %s already exists in mapping %s", deviceID, item.MappingID)
		}
		if item.LocalBindPort == candidate.LocalBindPort && netbind.SameBindAddress(item.LocalBindIP, candidate.LocalBindIP) {
			return fmt.Errorf("local bind %s:%d conflicts with mapping %s", candidate.LocalBindIP, candidate.LocalBindPort, item.MappingID)
		}
	}
	return nil
}
