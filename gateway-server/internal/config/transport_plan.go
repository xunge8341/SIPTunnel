...
func ResolveTransportPlan(mode NetworkMode) TunnelTransportPlan {
	...
	case NetworkModeSenderSIPReceiverSIP:
		plan.Notes = append(plan.Notes, "模式=SENDER_SIP__RECEIVER_SIP：映射业务全程仅走 SIP 链路，适合小请求 + 小响应。")
		plan.Warnings = append(plan.Warnings, "仅 SIP 模式下，请求体/响应体都受 SIP 报文大小限制。")
...
func effectiveSIPBodySizeLimit(cfg NetworkConfig) int {
	limit := cfg.SIP.MaxMessageBytes
	if strings.EqualFold(strings.TrimSpace(cfg.SIP.Transport), "UDP") {
		if cfg.TransportTuning.UDPControlMaxBytes > 0 && (limit <= 0 || cfg.TransportTuning.UDPControlMaxBytes < limit) {
			limit = cfg.TransportTuning.UDPControlMaxBytes
		}
	}
	return limit
}

func ResolveTransportPlanForConfig(cfg NetworkConfig) TunnelTransportPlan {
	plan := ResolveTransportPlan(cfg.Mode)
	effectiveLimit := effectiveSIPBodySizeLimit(cfg)
	if effectiveLimit <= 0 {
		return plan
	}
	if plan.RequestBodyTransport == TransportSIPBodyJSON || plan.RequestBodyTransport == TransportSIPBodyOnly {
		plan.RequestBodySizeLimit = effectiveLimit
	}
	if plan.ResponseBodyTransport == TransportSIPBodyJSON || plan.ResponseBodyTransport == TransportSIPBodyOnly {
		plan.ResponseBodySizeLimit = effectiveLimit
	}
	plan.Notes = append(plan.Notes, "SIP 小载荷限额展示为运行时有效值，已纳入 transport_tuning 收口。")
	return plan
}

func EffectiveInlineResponseBodyBudgetBytes(cfg NetworkConfig) int {
	return int(RecommendedInlineBodyBudgetBytes(cfg.TransportTuning))
}

func EffectiveResponseModePolicyLabel(cfg NetworkConfig) string {
	if strings.EqualFold(strings.TrimSpace(cfg.SIP.Transport), "UDP") {
		return "AUTO(budget_driven_inline_or_rtp)"
	}
	return "AUTO(header_then_body_finalization)"
}
