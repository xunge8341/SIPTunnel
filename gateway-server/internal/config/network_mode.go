...
const (
	// 新模型：按发送端 / 接收端表达链路能力。
	NetworkModeSenderSIPReceiverSIP    NetworkMode = "SENDER_SIP__RECEIVER_SIP"
...
func (m NetworkMode) Description() string {
	...
	case NetworkModeSenderSIPReceiverSIP:
		return "本级域/发起侧: SIP --> | <-- SIP : 级联对端/应答侧；仅走 SIP 链路，适合小请求 + 小响应"
	case NetworkModeSenderSIPReceiverRTP:
		return "本级域/发起侧: SIP --> | <-- RTP : 级联对端/应答侧；适合小请求 + 大响应"
	case NetworkModeSenderSIPReceiverSIPRTP:
		return "本级域/发起侧: SIP --> | <-- SIP&RTP : 级联对端/应答侧；双向小报文 + 下行大载荷"
	case NetworkModeSenderSIPRTPReceiverAll:
		return "本级域/发起侧: SIP&RTP --> | <-- SIP&RTP : 级联对端/应答侧；双向大载荷"
	default:
...
func DeriveCapability(mode NetworkMode) Capability {
	...
	case NetworkModeSenderSIPReceiverSIP:
		return Capability{
			...
			SupportsLargeResponseBody:       false,
...
var knownNetworkModes = map[NetworkMode]struct{}{
	NetworkModeSenderSIPReceiverSIP:    {},
