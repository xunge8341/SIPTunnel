...
func TestDeriveCapability(t *testing.T) {
	tests := []struct {
		...
		{
			name: "sip only small payload",
			mode: NetworkModeSenderSIPReceiverSIP,
			want: Capability{
...
func TestNetworkModeValidate(t *testing.T) {
	if err := NetworkModeSenderSIPReceiverSIP.Validate(); err != nil {
		t.Fatalf("new sip-only mode should be valid: %v", err)
	}
