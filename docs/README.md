...
以上菜单均为一级入口，不再保留二级中转页。

## 当前有效文档（Source of Truth）

以下文档属于**当前有效基线**，发生冲突时以它们为准：

- `README.md`：仓库级产品与术语总览
- `docs/ENGINEERING_GUARDRAILS.md`：固定原则、职责边界与禁止项
- `docs/design.md`：主设计说明
- `docs/BACKEND_CHAIN_SEQUENCE_AND_OPTIMIZATION_20260321.md`：后台真实执行链路、时序图与后续优化点
- `docs/OVERALL_CLEANUP_REVIEW_20260321.md`：当前清洁回顾、结构事实与下一轮优先级
- `docs/operations.md`：运维操作手册
- `docs/DEPLOYMENT_AND_OPERATIONS.md`：部署/升级/回滚
- `docs/CONFIGURATION_CLEANUP.md`：配置职责边界
- `docs/REVIEW_AND_CLEANUP_20260318.md`：本轮全盘 Review 结论与遗留问题
- 当前运行态策略补充：大响应交付只保留 `stream_primary / range_primary / adaptive_segmented_primary / fallback_segmented` 四类；目录暴露状态只保留 `MANUAL / UNEXPOSED`。

以 `docs/gb28181-phase*.md`、`gateway-server/docs/gb28181-phase*.md` 命名的文档视为**阶段性归档文档**，用于追溯，不作为当前 UI/代码/脚本的首要事实来源。
...
- `local_bind_port`：会拦截浏览器已知受限端口（例如 `6666`），避免配置后触发浏览器 `ERR_UNSAFE_PORT` 但服务端无请求日志。
...
- 隧道配置 API：`GET /api/tunnel/config`、`POST /api/tunnel/config`
  - 配置以 GB/T 28181 注册与心跳为核心：`channel_protocol`、`connection_initiator`、`heartbeat_interval_sec`、`register_retry_count`、`register_retry_interval_sec`；`local_device_id`/`peer_device_id` 为节点配置派生只读字段。
  - 页面只读展示 `registration_status`、`last_register_time`、`last_heartbeat_time`、`heartbeat_status` 与 `supported_capabilities`。
  - `request_channel/response_channel` 改为系统自动推导（SIP/RTP），不再作为运维高频编辑项。
...
- 后台链路与时序图：`docs/BACKEND_CHAIN_SEQUENCE_AND_OPTIMIZATION_20260321.md`
- 运维手册：`docs/operations.md`

## 迁移与回归（Round 10）

- 迁移说明：`docs/migration-round10.md`
- 推荐回归命令：
...
- 部署与运维操作说明：`DEPLOYMENT_AND_OPERATIONS.md`


## 阶段一严格模式验收

- 验收与抓包清单：`docs/GB28181_PHASE1_ACCEPTANCE_AND_PCAP_CHECKLIST_20260318.md`
- 源码静态验收脚本：`scripts/acceptance/verify_phase1_strict_source.sh`
- 一键验收编排：`scripts/acceptance/run_phase1_strict_acceptance.sh`
- 抓包控制脚本：`scripts/acceptance/pcap_capture.sh`
- 抓包核对脚本：`scripts/acceptance/verify_gb28181_pcap.sh`
