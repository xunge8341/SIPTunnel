...
with open(sys.argv[1],encoding='utf-8') as f:
  m=json.load(f)
print(m.get('embedded_at_local') or m.get('embedded_at_utc') or '')
PY
)"
...
with open(sys.argv[1],encoding='utf-8') as f:
  m=json.load(f)
print(m.get('ui_source_latest_write_local') or m.get('ui_source_latest_write_utc') or '')
PY
)"
  ...
  echo "[build] UI embedded_at_local: $embedded_at"
  ...
  if [ -n "$latest_ui_epoch" ] && [ -n "$embedded_at" ]; then
    embedded_epoch="$(date -d "$embedded_at" +%s)"
...
sync_go_module_graph() {
  echo "[build] syncing Go module graph (go mod tidy -compat=1.23.0)"
  (
    cd "$SERVER_DIR"
