...
$UiGuardReport = Join-Path $RootDir 'artifacts/acceptance/ui-delivery-guard-latest.json'
...
$guardMode = if ($CheckOnly) { 'verify' } else { 'repair' }
Write-Host "[ui-build] running UI delivery guard (mode: $guardMode)"
node $UiGuardScript --mode $guardMode --allow-missing-embedded-metadata --report $UiGuardReport
if ($LASTEXITCODE -ne 0) {
  throw "UI delivery guard failed with exit code $LASTEXITCODE. Please reconcile legacy UI drift before building."
}
Write-Host "[ui-build] delivery guard report: $UiGuardReport"

if ($CheckOnly) {
  Write-Host '[ui-build] check-only mode completed; manifest, guardrail, and toolchain validation passed'
  exit 0
}
