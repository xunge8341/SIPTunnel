...
run_gate "protocol_codec_tests" bash -lc "cd gateway-server && go test ./internal/protocol/sip ./internal/protocol/rtpfile ./internal/control -count=1"
run_gate "e2e_smoke" bash -lc 'cd scripts && ./regression/run.sh smoke'
run_gate "repo_consistency" bash -lc './scripts/check-consistency.sh'
run_gate "ui_quality" bash -lc "cd gateway-ui && npm run lint && npm run typecheck && npm run test && npm run build"
...
run_gate "benchmark_smoke" bash -lc "cd gateway-server && GOMAXPROCS=1 go test ./internal/protocol/sip ./internal/security ./internal/protocol/rtpfile ./internal/service/httpinvoke -run '^$' -bench 'Benchmark(SIPJSONDecodeValidate|Signer(Sign|Verify)|File(Split|Assemble)|HTTP(MapByTemplate|InvokeWrapper))$' -benchmem -benchtime=100ms -count=1"
