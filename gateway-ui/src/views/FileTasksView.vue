<template>
  <a-space direction="vertical" size="middle" style="width: 100%">
    ...
    <a-card title="文件任务列表">
      <a-table :columns="columns" :data-source="result.list" row-key="id" :pagination="pagination" @change="onTableChange">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'checksumPassed'">
            <a-badge :status="record.checksumPassed ? 'success' : 'error'" :text="record.checksumPassed ? '通过' : '失败'" />
          </template>
...
import type { FileTask, TaskListFilters, TaskListResult } from '../types/gateway'

const router = useRouter()
const filters = reactive<TaskListFilters>({})
const result = ref<TaskListResult<FileTask>>({ list: [], total: 0, page: 1, pageSize: 10 })
...
const columns = [
  ...
  { title: '传输进度', key: 'progress' },
  { title: '缺片数', dataIndex: 'missingShards', key: 'missingShards' },
  { title: '重传轮次', dataIndex: 'retryRounds', key: 'retryRounds' },
  { title: '校验结果', dataIndex: 'checksumPassed', key: 'checksumPassed' },
  { title: '操作', key: 'action' }
]
...
const loadData = async (page = pagination.value.current, pageSize = pagination.value.pageSize) => {
  result.value = await gatewayApi.fetchFileTasks(filters, page, pageSize)
...
const goDetail = (id: string) => {
  router.push({ name: 'task-detail', params: { taskKind: 'file', id } })
}
